...
def persist_topic_index_json(batch_name: str, topic_index: dict) -> str:
    import json, re, os
...
async def get_or_create_forum_topic(bot, chat_id: int, title: str) -> Optional[int]:
...
keyboard = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton(text="🛠️ Help", url="https://t.me/TgXWarriors")        ],
    ]
)
...
async def getcookies_handler(client: Client, m: Message):
    try:
        ...
        , **({'message_thread_id': thread_id} if 'thread_id' in locals() and thread_id else {}) )
    except Exception as e:
        await m.reply_text(f"⚠️ An error occurred: {str(e)}")
    ...
    try:
        if m.chat.type == "channel":
            ...
            await m.reply_photo(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📚 Help", url="https://t.me/TgXWarriors")]])
            )
...
async def unauthorized_handler(client, message: Message):
    await message.reply(
        ...
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("💫 Get Premium Access", url="https://t.me/TgXWarriors")
        ]])
    )
...
async def txt_handler(bot: Client, m: Message):  
    # ADD-ON: forum topic map + topic index storage
    topic_threads = {}
    topic_index = {}
    try:
    ...
    editable = await m.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [[
                InlineKeyboardButton("👑 Join Premium Group", url="https://t.me/TgXWarriors")
            ]]
        )
    )
    ...
    copy = await bot.send_document(OWNER_ID, x)

                        # ADD-ON: append to topic_index & persist
                        try:
                            permalink = getattr(copy, "link", None) or (f"https://t.me/c/{str(channel_id)[4:]}/{copy.id}" if str(channel_id).startswith("-100") else "")
                            if topic not in topic_index:
                                topic_index[topic] = []
                            title_for_index = f"{topic} | {name1}.pdf" if 'name1' in locals() else (name if 'name' in locals() else 'Document')
                            topic_index[topic].append({"title": title_for_index, "link": permalink})
                            try_db_save_index(b_name if 'b_name' in locals() else (batch if 'batch' in locals() else 'Batch'), topic_index)
                        except Exception as e:
                            print("Index append error:", e)
    ...
    f"<pre>🏷 ᴛᴏᴘɪᴄ : {topic}</pre>
"
    f"<pre>🏷 ᴛᴏᴘɪᴄ : {topic}</pre>
"
    f"<pre>📘 ʙᴀᴛᴄʜ : {b_name}</pre>\n"
    f"━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    f"<b>👤 Uploaded By: 🔥 <a href=\"https://t.me/MrsUC\">𝙈𝙧𝙨.𝙐𝘾 ❣️</a></b>\n"                
                ...
                if "drive" in url:
                    try:
                        ...
                        copy = copy = await bot.send_document(chat_id=channel_id,document=ka, caption=cc1)
                        ...
                        for attempt in range(max_retries):
                            try:
                                ...
                                if response.status_code == 200:
                                    ...
                                    copy = copy = await bot.send_document(chat_id=channel_id, document=f'{name}.pdf', caption=cc1)
                    ...
                    try:
                        ...
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        os.system(download_cmd)
                        copy = await bot.send_document(chat_id=channel_id, document=f'{name}.{ext}', caption=cc1)
    ...
    # ADD-ON: persist & post topic-wise index AFTER uploads
    try:
        if topic_index:
            try_db_save_index(b_name if 'b_name' in locals() else (batch if 'batch' in locals() else 'Batch'), topic_index)
            persist_topic_index_json(b_name if 'b_name' in locals() else (batch if 'batch' in locals() else 'Batch'), topic_index)
            # Post simple grouped index
            lines = ["<b>🧭 Topic-wise Index</b>",""]
            for tpc, items in topic_index.items():
                lines.append(f"<b>• {tpc}</b>")
                for it in items:
                    title = it.get("title","")
                    url = it.get("link","")
                    short = (title[:80]+"…") if len(title)>80 else title
                    lines.append(f"  ─ <a href='{url}'>{short}</a>")
                lines.append("")
            text = "\n".join(lines)
            for chunk in [text[i:i+3500] for i in range(0, len(text), 3500)]:
                await client.send_message((channel_id if 'channel_id' in locals() else m.chat.id), chunk, disable_web_page_preview=True)
    except Exception as e:
        print("Index post failed:", e)
    ...
    try:
            ...
            try:
                ...
                if "drive" in url:
                    try:
                        ka = await helper.download(url, name)
                        copy = copy = await bot.send_document(chat_id=m.chat.id,document=ka, caption=cc1)
                            ...
                            try:
                                ...
                                if response.status_code == 200:
                                    with open(f'{name}.pdf', 'wb') as file:
                                        file.write(response.content)
                                    await asyncio.sleep(retry_delay)  # Optional, to prevent spamming
                                    copy = copy = await bot.send_document(chat_id=m.chat.id, document=f'{name}.pdf', caption=cc1)
